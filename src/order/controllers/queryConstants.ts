export const SELECT_ORDER_BY_ID =
  'SELECT o.id, JSON_OBJECT("id", o.tableId, "number", t.number) as table_order, JSON_OBJECT("id", o.userId, "name", u.name, "email", u.email) as user, JSON_OBJECT("id", os.id, "description", os.description) as state, o.date_created, o.total, o.feedback, o.score, CASE WHEN o.employeeId IS NULL THEN NULL ELSE JSON_OBJECT("id", o.employeeId, "name", emp.name) END as employee,  JSON_ARRAYAGG(JSON_OBJECT("productId", od.productId, "promotionId", od.promotionId, "quantity", od.quantity, "unitPrice", od.unitPrice, "comments", od.comments)) AS orderDetails, ti.payment_method FROM orders AS o INNER JOIN orderDetail od ON od.orderId = o.id INNER JOIN orderState os ON os.id = o.idState INNER JOIN users u ON o.userId = u.id LEFT JOIN users emp ON o.employeeId = emp.id INNER JOIN tables t ON o.tableId = t.id LEFT JOIN tickets ti ON ti.order_id = o.id WHERE o.id = ? GROUP BY o.id, o.tableId, t.number, o.userId, u.name, u.email, os.id, os.description, o.date_created, o.total, o.feedback, o.score, o.employeeId,emp.name, ti.payment_method ORDER BY date_created';
...
export const GET_ALL_ORDERS_USER_TABLE =
  'SELECT o.id, o.userId, st.description as status FROM orders o INNER JOIN orderState st ON st.id = o.idState WHERE userId = ?  AND tableId = ? AND DATE(o.date_created) = DATE(CONVERT_TZ(NOW(), "+00:00", "-03:00"))';
