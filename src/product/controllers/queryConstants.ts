export const SELECT_PRODUCT_BY_ID =
  "SELECT p.name, p.description, p.image, c.description AS category, p.baja, pre.price FROM products AS p INNER JOIN productsType AS c ON p.idCat = c.idProductType INNER JOIN productPrice AS pre ON pre.idProd = p.id WHERE pre.valid_from = ( SELECT MAX(valid_from) FROM productPrice WHERE idProd = p.id ) AND p.id = ?";
...
export const UPDATE_PRODUCT =
  "UPDATE products p INNER JOIN productPrice pre ON p.id = pre.idProd SET p.name = IFNULL(?, p.name), p.description = IFNULL(?, p.description), p.image = IFNULL(?, p.image), p.idCat = IFNULL(?, p.idCat), p.baja = IFNULL(?, p.baja), pre.price = IFNULL(?, pre.price) WHERE p.id = ?";
