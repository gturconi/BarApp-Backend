export const SELECT_PRODUCT_BY_ID =
  "SELECT p.id, p.name, p.description, p.image, c.description AS category, p.baja, p.stock, pre.price FROM products AS p INNER JOIN productsType AS c ON p.idCat = c.id INNER JOIN productPrice AS pre ON pre.idProd = p.id WHERE pre.valid_from = ( SELECT MAX(valid_from) FROM productPrice WHERE idProd = p.id ) AND p.id = ?";
