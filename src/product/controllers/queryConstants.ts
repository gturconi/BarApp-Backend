export const SELECT_PRODUCT_BY_ID =
  'SELECT p.id, p.name, p.description, p.image, c.id AS idCat, c.description AS category, p.baja, p.stock, pre.price, (SELECT CASE WHEN COUNT(*) > 0 THEN JSON_ARRAYAGG(JSON_OBJECT("discount", discount, "days_of_week", (SELECT JSON_ARRAYAGG(day_of_week) FROM promotionDays WHERE promotion_id = promotions.id))) ELSE NULL END FROM products_promotions JOIN promotions ON products_promotions.idProm = promotions.id WHERE idProd = p.id AND (valid_from IS NULL OR CURDATE() BETWEEN valid_from AND valid_to OR valid_to IS NULL) AND discount IS NOT NULL AND baja = 0 AND price IS NULL GROUP BY idProd) AS promotions FROM products AS p INNER JOIN productsType AS c ON p.idCat = c.id INNER JOIN productPrice AS pre ON pre.idProd = p.id WHERE pre.valid_from = (SELECT MAX(valid_from) FROM productPrice WHERE idProd = p.id) AND p.id = ?';
