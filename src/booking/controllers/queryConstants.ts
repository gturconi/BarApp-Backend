...
export const SELECT_BOOKINGS =
  'SELECT b.id, b.date_hour, b.reason, JSON_OBJECT("id", b.userId, "name", u.name) as user, quota , JSON_OBJECT("id", b.stateId, "description", bs.description) as state, b.bookingDayId	 FROM bookings b INNER JOIN users u ON u.id = b.userId INNER JOIN bookingState bs ON bs.id = b.stateId WHERE date_hour LIKE CONCAT("%", ?, "%") OR u.name LIKE CONCAT("%", ?, "%") OR bs.description LIKE CONCAT("%", ?, "%") ORDER BY date_hour DESC, b.id DESC LIMIT ?, ?';
...
export const GET_FUTURE_BOOKINGS =
  'SELECT * FROM bookings WHERE date_hour > DATE(CONVERT_TZ(NOW(), "+00:00", "-03:00")) AND userId = ? ORDER BY date_hour DESC, id DESC';

export const UPDATE_BOOKING_STATE =
  "UPDATE bookings SET stateId = ? WHERE id = ?";

export const UPDATE_CANCEL_BOOKING_STATE =
  "UPDATE bookings SET stateId = ? , reason = ? WHERE id = ?";
