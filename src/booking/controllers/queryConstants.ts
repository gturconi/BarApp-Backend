...
export const SELECT_AVAILABLE_BOOKING_DAY =
  "SELECT * FROM bookingDays where day_of_week = ? AND ? >= TIME(init_hour) AND ? <= TIME(end_hour)";
...
export const SELECT_BOOKINGS =
  "SELECT b.id, b.date_hour, b.quota, b.userId, b.bookingDayId, b.stateId, u.email, s.description FROM bookings AS b INNER JOIN users AS u ON b.userId = u.id INNER JOIN bookingState AS s ON b.stateId = s.id";
...
export const GET_FUTURE_BOOKINGS =
  'SELECT b.id, b.date_hour, b.userId, b.stateId, b.bookingDayId, b.quota, s.description FROM bookings AS b INNER JOIN bookingState AS s ON b.stateId = s.id WHERE date_hour > DATE(CONVERT_TZ(NOW(), "+00:00", "-03:00")) AND userId = ? ';
